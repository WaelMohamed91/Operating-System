...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32  start_address_page_table =(uint32) kmalloc(PAGE_SIZE);
	tlbflush();

	memset((uint32 *)start_address_page_table , 0, PAGE_SIZE);

	uint32 physical = kheap_physical_address(start_address_page_table);

	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(physical,
                PERM_PRESENT | PERM_USER | PERM_WRITEABLE);


	//change this "return" according to your answer
	// tlbflush();
	return (uint32 *)start_address_page_table;
}
...
uint32 actual_size = size /PAGE_SIZE ;
	...
	for (int i =  0;  i < actual_size ; i++ ) {
		...
		int res = pf_add_empty_env_page( e , virtual_address, 0 ) ;
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for(uint32 i=virtual_address; i<virtual_address+size ;i+=PAGE_SIZE)
	{
		...
		uint32 perm=pt_get_page_permissions(e,i);
		uint32 isbuff =perm&PERM_BUFFERED;

		uint32 * ptr_PT = NULL;
		get_page_table(e->env_page_directory,(void*)i,&ptr_PT);
		//if ( ptr_PT != NULL)
		//{
		if((perm&PERM_BUFFERED) == PERM_BUFFERED)
		{
			uint32*ptr_table=NULL;
			struct Frame_Info* ptr_frame = get_frame_info(e->env_page_directory,(void*)i,&ptr_table);

			//cprintf("buffer\n");
			uint32 ismod = perm&PERM_MODIFIED;
			...
			//c) Set its entry in the page table to NULL to indicate that this page is no longer exists
		...
		// a)Clear the entry inside the page table to indicate that the page is no longer exists
		// b)Update the working sets after removing


		for(int j =0;j<e->page_WS_max_size;j++)
		{
			uint32 vir_WS= env_page_ws_get_virtual_address(e,j);

			if( vir_WS >= virtual_address  && vir_WS < virtual_address +size  )
			{
		...
		uint32 virtual_address_temp = virtual_address ;
		virtual_address_temp = ROUNDDOWN (virtual_address_temp , PAGE_SIZE*1024 ) ;

		for (uint32 i = virtual_address_temp ; i < virtual_address_temp+size ;  i+= PAGE_SIZE  ) {
			...
			if(ptr_page_table!=NULL)
			{
				...
				{
					if (ptr_page_table[m] != 0)
					{delete = 0 ; break ; }

					//uint32 entry = ptr_page_table[m]&PERM_PRESENT;
					//if ((ptr_page_table[m]&PERM_PRESENT) == PERM_PRESENT)
					//{delete = 0 ; break ; }
				}

				if (delete) {
				//	cprintf("page table removed\n");
					pd_clear_page_dir_entry(e,i);
					//e->env_page_directory[PDX(i)] = 0 ; // Remove Empty page table
					unmap_frame(e->env_page_directory , (void*) ptr_page_table) ;
