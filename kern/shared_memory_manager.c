...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	for(int i = 0 ;i < MAX_SHARES; i++)
	{
		if(shares[i].ownerID == ownerID && strcmp(shares[i].name , shareName) == 0)
		{
	...
	struct Share *allocateobject ;
	int SharedObjectID = -1  ;
	SharedObjectID	=allocate_share_object(&allocateobject);

	if (SharedObjectID == E_NO_SHARE  || SharedObjectID == -1  )
		return E_NO_SHARE;
	...
	allocateobject->empty = 0;
	allocateobject->isWritable = isWritable;
	allocateobject->ownerID = ownerID;
	allocateobject->references = 1;
	allocateobject->size = size;
	strcpy(allocateobject->name,shareName);
	...
	for (uint32 i=0;i<NumOfPages;i++)
	{
		...
		//if(isWritable)
			map_frame(myenv->env_page_directory,ptr,(void*)virtual_address,PERM_WRITEABLE|PERM_PRESENT|PERM_USER);
		//else
			//map_frame(myenv->env_page_directory,ptr,(void*)virtual_address,PERM_PRESENT|PERM_USER);
		...
		add_frame_to_storage(allocateobject->framesStorage,ptr,i);

		virtual_address+=PAGE_SIZE;

	}

	shares[SharedObjectID] = *allocateobject;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	//	1) Get the shared object from the "shares" array (use get_share_object_ID())
     int objectID =	get_share_object_ID(ownerID,shareName);
     if(objectID == E_SHARED_MEM_NOT_EXISTS)
    	 return E_SHARED_MEM_NOT_EXISTS;

     int Pages = shares[objectID].size/PAGE_SIZE;
     for(uint32 i = 0;i<Pages;i++)
     {
    	struct Frame_Info * ptr_frame = get_frame_from_storage(shares[objectID].framesStorage,i);
		if(shares[objectID].isWritable)
			map_frame(myenv->env_page_directory,ptr_frame,(void*)virtual_address,PERM_WRITEABLE|PERM_PRESENT|PERM_USER);
		else
			map_frame(myenv->env_page_directory,ptr_frame,(void*)virtual_address,PERM_PRESENT|PERM_USER);

		virtual_address+=PAGE_SIZE;
     }
     shares[objectID].references++;
